...
----
We add a stateMap named `coins`. The statemap will be entered when there are outstanding coins.
        ...
        val definition = stateMachine(
            ...
            stateMap("coins", setOf(PayingTurnstileStates.COINS)) {
                state(PayingTurnstileStates.COINS) {
                    ...
                    popTransition(PayingTurnstileEvents.COIN to PayingTurnstileStates.UNLOCKED,
                        guard = { args ->
                            val value = args[0] as Int
                    ...
                    popTransition(PayingTurnstileEvents.COIN to PayingTurnstileStates.UNLOCKED) { args ->
                        val value = args[0] as Int
                        coin(value)
                        unlock()
                        reset()
