...
The Secure Turnstile is an example of where a timeout would be useful. Allowing override should only be active for a limited period.
...
The timeout will trigger an optional transition and action.

A requirement for timeout handling is correct concurrent handling of events.
