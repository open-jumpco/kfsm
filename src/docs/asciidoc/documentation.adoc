...
[source,kotlin]
----
onEntry { fromState, targetState, args ->
    entryAction()
}
----
...
[source,kotlin]
----
onExit { fromState, targetState, args ->
    exitAction()
}
----
==== `onEvent`
...
[source,kotlin]
----
onEvent(Event.EVENT) { args -> // default internal state action for given event
    someFunction()
}

onEvent(Event.EVENT to State.STATE) { args-> // default external state action for given event
    anotherFunction()
}
----
...
onEntry { fromState, targetState, args -> // state entry action
...
[source,kotlin]
----
onExit { fromState, targetState, args -> // state exit action
...
[source,kotlin]
----
onEvent(Event.EV1, guard = { flag == 1 }) { args -> // internal transition with guard expression
}
onEvent(Event.EV1 to State.S2, guard = { flag == 2}) { args -> // external transition with guard expression
}
onEvent(Event.EV1) { args -> // internal transition
}
onEvent(Event.EV2 to State.S2) { args -> // external transition
}
----
==== `onEventPush`
