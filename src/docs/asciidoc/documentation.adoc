...
whenState(State.STATE) {
    ...
    timeout(State.S1, timeout, gaurd = { expression }) { // transition to S1 when timeout is triggered and guard is true
    }
    timeoutPop(State.S1, timeout, gaurd = { expression }) { // transition to S1 when timeout is triggered and guard is true
    }
    timeoutPush(State.S1, "mapName",  timeout, gaurd = { expression }) { // transition to S1 when timeout is triggered and guard is true
    }
}
----
