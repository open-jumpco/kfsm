...
class DslStateMapEventHandler<S, E, C, A, R>(
     ...
     */
    fun onEvent(event: EventState<E, S>, action: StateAction<C, A, R>?): DslStateMapEventHandler<S, E, C, A, R> {
     ...
     * @param action The optional action that may be executed
     */
    fun onEvent(event: E, action: StateAction<C, A, R>?): DslStateMapEventHandler<S, E, C, A, R> {
     ...
     * @param action The optional action that may be executed
     */
    fun onEvent(event: E, guard: StateGuard<C, A>, action: StateAction<C, A, R>?): DslStateMapEventHandler<S, E, C, A, R> {
    ...
    fun onEventPop(event: E, action: StateAction<C, A, R>?): DslStateMapEventHandler<S, E, C, A, R> {
        fsm.popTransition(currentState, event, null, null, action)
        return this
    }

    fun onEventPop(event: Pair<E, S>, action: StateAction<C, A, R>?): DslStateMapEventHandler<S, E, C, A, R> {
    ...
    fun onEventPop(event: E, guard: StateGuard<C, A>, action: StateAction<C, A, R>?): DslStateMapEventHandler<S, E, C, A, R> {
    ...
    fun automatic(targetState: S, action: StateAction<C, A, R>?) {
        fsm.automatic(currentState, targetState, action)
    }

    fun automatic(targetState: S, guard: StateGuard<C, A>, action: StateAction<C, A, R>?) {
        fsm.automatic(currentState, targetState, guard, action)
    }

    fun automaticPop(targetState: S, action: StateAction<C, A, R>?) {
        fsm.automaticPop(currentState, targetState, action)
    }

    fun automaticPop(action: StateAction<C, A, R>?) {
        fsm.automaticPop(currentState, action)
    }

    fun automaticPop(targetMap: String, targetState: S, action: StateAction<C, A, R>?) {
        fsm.automaticPop(currentState, targetMap, targetState, action)
    }

    fun automaticPop(targetState: S, guard: StateGuard<C, A>, action: StateAction<C, A, R>?) {
        fsm.automaticPop(currentState, targetState, guard, action)
    }

    fun automaticPop(guard: StateGuard<C, A>, action: StateAction<C, A, R>?) {
        fsm.automaticPop(currentState, guard, action)
    }

    fun automaticPop(targetMap: String, targetState: S, guard: StateGuard<C, A>, action: StateAction<C, A, R>?) {
        fsm.automaticPop(currentState, targetMap, targetState, guard, action)
    }

    fun automaticPush(targetMap: String, targetState: S, action: StateAction<C, A, R>?) {
        fsm.automaticPush(currentState, targetMap, targetState, action)
    }

    fun automaticPush(targetMap: String, targetState: S, guard: StateGuard<C, A>, action: StateAction<C, A, R>?) {
